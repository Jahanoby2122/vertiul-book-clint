...
const Navbar = () => {
  ...
  const handleLogout = ()=>{
    logoutUser()
      .then(result=>{
        console.log(result)
        navigate('/')
        toast.error("SignOut succesfully")
    ...
    return (
        <div className=''>
            <div className="navbar fixed top-0 left-0 b  bg-white border-b border-[#9128fb] shadow z-50 ">
  <div className="navbar-start">
    ...
    <img className='w-[120px]' src={logoImg} alt="" />
  </div>
  ...
  <div className="navbar-end space-x-2">
   {
    user? <button onClick={handleLogout} className='btn'>Logout</button>: <div> <Link className='btn' to={`/auth/signup`}>signup</Link>
    <Link className='btn' to={`/auth/login`}>Login</Link></div>
   }
  </div>
</div>
        </div>
